...
public class Giga extends TamableAnimal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Giga(EntityType<? extends Giga> entityType, Level level) {
    ...
    public static final Predicate<LivingEntity> PREY_SELECTOR = (entity) -> {
        ...
        if (entity.getType() == EntityTypes.GIGA_ENTITY.get()) {
            return false;
        }
        return true;
    };
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Giga)) {
            return false;
        } else {
            return this.canBeParent() && ((Giga)animal).canBeParent();
        }
    }
    @Override
    public Giga getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.GIGA_ENTITY.get().create(level);
    }
