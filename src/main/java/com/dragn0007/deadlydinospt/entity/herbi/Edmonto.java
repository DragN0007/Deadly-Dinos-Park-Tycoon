...
public class Edmonto extends TamableAnimal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Edmonto(EntityType<? extends Edmonto> entityType, Level level) {
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Edmonto)) {
            return false;
        } else {
            return this.canBeParent() && ((Edmonto)animal).canBeParent();
        }
    }
    @Override
    public Edmonto getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.EDMONTO_ENTITY.get().create(level);
    }
