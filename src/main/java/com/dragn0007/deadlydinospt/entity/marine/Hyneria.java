...
public class Hyneria extends AbstractPassiveMarineDino implements IAnimatable {
    ...
    public boolean canMate(WaterDino animal) {
        if (animal == this || !(animal instanceof Hyneria)) {
            return false;
        } else {
            return this.canBeParent() && ((Hyneria)animal).canBeParent();
        }
    }

    @Override
    public Hyneria getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.HYNERIA_ENTITY.get().create(level);
    }
