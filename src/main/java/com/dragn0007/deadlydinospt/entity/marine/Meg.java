...
public class Meg extends AbstractMarineDino implements IAnimatable {
    ...
    public boolean canMate(WaterDino animal) {
        if (animal == this || !(animal instanceof Meg)) {
            return false;
        } else {
            return this.canBeParent() && ((Meg)animal).canBeParent();
        }
    }

    @Override
    public Meg getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.MEG_ENTITY.get().create(level);
    }
