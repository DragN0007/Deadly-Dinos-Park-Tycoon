...
public class Livy extends AbstractNeutralMarineDino implements IAnimatable {
    ...
    public boolean canMate(WaterDino animal) {
        if (animal == this || !(animal instanceof Livy)) {
            return false;
        } else {
            return this.canBeParent() && ((Livy)animal).canBeParent();
        }
    }

    @Override
    public Livy getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.LIVY_ENTITY.get().create(level);
    }
