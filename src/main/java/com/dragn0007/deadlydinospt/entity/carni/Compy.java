...
public class Compy extends TamableAnimal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Compy(EntityType<? extends Compy> entityType, Level level) {
    ...
    public static final Predicate<LivingEntity> PREY_SELECTOR = (entity) -> {
        ...
        if (entity.getType() == EntityTypes.COMPY_ENTITY.get()) {
            return false;
        }
        return true;
    };
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Compy)) {
            return false;
        } else {
            return this.canBeParent() && ((Compy)animal).canBeParent();
        }
    }
    @Override
    public Compy getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.COMPY_ENTITY.get().create(level);
    }
