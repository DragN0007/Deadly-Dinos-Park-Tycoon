...
public class Tarbo extends TamableAnimal implements IAnimatable {
    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Tarbo(EntityType<? extends Tarbo> entityType, Level level) {
    ...
    public static final Predicate<LivingEntity> PREY_SELECTOR = (entity) -> {
        ...
        if (entity.getType() == EntityTypes.TARBO_ENTITY.get()) {
            return false;
        }
        return true;
    };
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Tarbo)) {
            return false;
        } else {
            return this.canBeParent() && ((Tarbo)animal).canBeParent();
        }
    }
    @Override
    public Tarbo getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.TARBO_ENTITY.get().create(level);
    }
