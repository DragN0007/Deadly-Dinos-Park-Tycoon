...
public class Utah extends TamableAnimal implements IAnimatable {
    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Utah(EntityType<? extends Utah> entityType, Level level) {
    ...
    public static final Predicate<LivingEntity> PREY_SELECTOR = (entity) -> {
        ...
        if (entity.getType() == EntityTypes.UTAH_ENTITY.get()) {
            return false;
        }
        return true;
    };
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Utah)) {
            return false;
        } else {
            return this.canBeParent() && ((Utah)animal).canBeParent();
        }
    }
    @Override
    public Utah getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.UTAH_ENTITY.get().create(level);
    }
