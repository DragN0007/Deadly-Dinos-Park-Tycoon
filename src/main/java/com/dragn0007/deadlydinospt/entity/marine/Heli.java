...
public class Heli extends AbstractMarineDino implements IAnimatable {
    ...
    public boolean canMate(WaterDino animal) {
        if (animal == this || !(animal instanceof Heli)) {
            return false;
        } else {
            return this.canBeParent() && ((Heli)animal).canBeParent();
        }
    }

    @Override
    public Heli getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.HELI_ENTITY.get().create(level);
    }
