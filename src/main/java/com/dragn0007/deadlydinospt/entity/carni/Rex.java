...
public class Rex extends TamableAnimal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Rex(EntityType<? extends Rex> entityType, Level level) {
    ...
    public static final Predicate<LivingEntity> PREY_SELECTOR = (entity) -> {
        ...
        if (entity.getType() == EntityTypes.REX_ENTITY.get()) {
            return false;
        }
        return true;
    };
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Rex)) {
            return false;
        } else {
            return this.canBeParent() && ((Rex)animal).canBeParent();
        }
    }
    @Override
    public Rex getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.REX_ENTITY.get().create(level);
    }
