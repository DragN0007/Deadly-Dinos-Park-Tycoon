...
public class Dilo extends TamableAnimal implements IAnimatable {
    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Dilo(EntityType<? extends Dilo> entityType, Level level) {
    ...
    public static final Predicate<LivingEntity> PREY_SELECTOR = (entity) -> {
        ...
        if (entity.getType() == EntityTypes.DILO_ENTITY.get()) {
            return false;
        }
        return true;
    };
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Dilo)) {
            return false;
        } else {
            return this.canBeParent() && ((Dilo)animal).canBeParent();
        }
    }
    @Override
    public Dilo getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.DILO_ENTITY.get().create(level);
    }
