...
public class Stego extends TamableAnimal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Stego(EntityType<? extends Stego> entityType, Level level) {
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Stego)) {
            return false;
        } else {
            return this.canBeParent() && ((Stego)animal).canBeParent();
        }
    }
    @Override
    public Stego getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.STEGO_ENTITY.get().create(level);
    }
