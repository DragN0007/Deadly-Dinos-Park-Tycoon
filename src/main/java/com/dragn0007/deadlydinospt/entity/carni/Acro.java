...
public class Acro extends TamableAnimal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Acro(EntityType<? extends TamableAnimal> entityType, Level level) {
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Acro)) {
            return false;
        } else {
            return this.canBeParent() && ((Acro)animal).canBeParent();
        }
    }
    @Override
    public Acro getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.ACRO_ENTITY.get().create(level);
    }
