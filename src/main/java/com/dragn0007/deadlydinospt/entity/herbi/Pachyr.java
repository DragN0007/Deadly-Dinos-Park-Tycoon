...
public class Pachyr extends TamableAnimal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Pachyr(EntityType<? extends Pachyr> entityType, Level level) {
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Pachyr)) {
            return false;
        } else {
            return this.canBeParent() && ((Pachyr)animal).canBeParent();
        }
    }
    @Override
    public Pachyr getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.PACHYR_ENTITY.get().create(level);
    }
