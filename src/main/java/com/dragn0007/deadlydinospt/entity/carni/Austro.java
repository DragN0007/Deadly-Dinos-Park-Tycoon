...
public class Austro extends TamableAnimal implements IAnimatable {
    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Austro(EntityType<? extends Austro> entityType, Level level) {
   ...
   public boolean canBeParent() {
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Austro)) {
            return false;
        } else {
            return this.canBeParent() && ((Austro)animal).canBeParent();
        }
    }
    @Override
    public Austro getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.AUSTRO_ENTITY.get().create(level);
    }
