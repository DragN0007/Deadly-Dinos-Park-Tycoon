...
public class Mosa extends AbstractMarineDino implements IAnimatable {
    ...
    public boolean canMate(WaterDino animal) {
        if (animal == this || !(animal instanceof Mosa)) {
            return false;
        } else {
            return this.canBeParent() && ((Mosa)animal).canBeParent();
        }
    }

    @Override
    public Mosa getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.MOSA_ENTITY.get().create(level);
    }
