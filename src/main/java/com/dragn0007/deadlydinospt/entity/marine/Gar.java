...
public class Gar extends AbstractNeutralMarineDino implements IAnimatable {
    ...
    public boolean canMate(WaterDino animal) {
        if (animal == this || !(animal instanceof Gar)) {
            return false;
        } else {
            return this.canBeParent() && ((Gar)animal).canBeParent();
        }
    }

    @Override
    public Gar getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.GAR_ENTITY.get().create(level);
    }
