...
public class Deinoch extends TamableAnimal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Deinoch(EntityType<? extends Deinoch> entityType, Level level) {
    ...
    public static final Predicate<LivingEntity> PREY_SELECTOR = (entity) -> {
        ...
        if (entity.getType() == EntityTypes.DEINOCH_ENTITY.get()) {
            return false;
        }
        return true;
    };
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Deinoch)) {
            return false;
        } else {
            return this.canBeParent() && ((Deinoch)animal).canBeParent();
        }
    }
    @Override
    public Deinoch getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.DEINOCH_ENTITY.get().create(level);
    }
