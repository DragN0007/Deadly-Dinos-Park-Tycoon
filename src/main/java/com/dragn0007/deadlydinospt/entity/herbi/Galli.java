...
public class Galli extends TamableAnimal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Galli(EntityType<? extends Galli> entityType, Level level) {
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Galli)) {
            return false;
        } else {
            return this.canBeParent() && ((Galli)animal).canBeParent();
        }
    }
    @Override
    public Galli getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.GALLI_ENTITY.get().create(level);
    }
