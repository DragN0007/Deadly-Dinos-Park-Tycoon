...
import com.dragn0007.deadlydinospt.client.model.SauroModel;
...
public class Sauro extends TamableAnimal implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public Sauro(EntityType<? extends Sauro> entityType, Level level) {
    ...
    public boolean canMate(Animal animal) {
        if (animal == this || !(animal instanceof Sauro)) {
            return false;
        } else {
            return this.canBeParent() && ((Sauro)animal).canBeParent();
        }
    }
    @Override
    public Sauro getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return EntityTypes.SAURO_ENTITY.get().create(level);
    }
